...
/**
 * install_mysql.php
 * Interface d'installation de GTC pour un environnement mysql
 * Ce script fait partie de l'application GTC
 * Dernière modification : $Date: 2009-10-09 07:55:48 $
 * @author    Laurent Delineau <laurent.delineau@ac-poitiers.fr>
 * @copyright Copyright 2003-2008 Laurent Delineau
 * @link      http://www.gnu.org/licenses/licenses.html
 * @package   root
 * @version   $Id: install_mysql.php,v 1.9 2009-10-09 07:55:48 GTC Exp $
 * @filesource
 *
 * This file is part of GTC.
 *
 * GTC is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation; either version 2 of the License, or
 * (at your option) any later version.
 *
 * GTC is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with GTC; if not, write to the Free Software
		...
		if (mysqli_select_db($db, "$dbDb"))
		{
			...
			if (($test2 != 0) && ($test1 != 'no'))
			{
				...
				if ($etape == 5)
				{
					...
					echo "<p>";
					echo "<p>Vous pouvez maintenant commencer à utiliser le système de gestion de club ...</p>";
					...
					echo "<br /><center><a href = 'login.php'>Se connecter à GTC</a></center>";
				}
				else
				{
					echo "<h2>Espace interdit - GTC est déjà installé.</h2>";
				}
				end_html();
				die();
			}
...
else if ($etape == 3)
{
	...
	{
		echo "<p><b>Le serveur $dbsys contient plusieurs bases de données.<br />Sélectionnez celle dans laquelle vous voulez implanter GTC</b></p>\n";
		echo "<ul>\n";
		$bases = "";
		$checked = FALSE;
		for ($i = 0; $i < $n; $i++)
	...
	echo "<li>les tables GTC seront créées dans la base sélectionnée. Si celle-ci contient déjà des tables GTC, ces tables, ainsi que les données qu'elles contiennent, seront supprimées et remplacées par une nouvelle structure.</li>\n</ul>\n";
	echo "<div style=\"text-align:right;\"><input type='submit' class='fondl' name='Valider' value='Suivant &gt;&gt;' /></div>\n";
	echo "</div></form>\n";
	end_html();
}
...
else if ($etape == 1)
{
	...
	echo "<p>Vous devez avoir en votre possession les codes de connexion au serveur $dbsys. Si ce n'est pas le cas, contactez votre hébergeur ou bien l'administrateur technique du serveur sur lequel vous voulez implanter GTC.</p>";
