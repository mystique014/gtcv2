...
if (isset($area))
{
    if (empty($area_name))
    {
        $res = grr_sql_query("select area_name, access from grr_area where id=$area");
...
if(isset($area))
    echo "<td><a href=\"admin_edit_room.php?area_id=$area\">".get_vocab('addroom')."</a></td></tr>";
else
    echo "<td>&nbsp;</td></tr>";
...
if (grr_sql_count($res) != 0) {
    ...
    for ($i = 0; ($row = grr_sql_row($res, $i)); $i++) {
        ...
        if(authGetUserLevel(getUserName(),$row[0],'area') >= 4) {
            echo "<tr>";
            if ($row[2]=='r')
                echo "<td><a href='admin_access_area.php?area=$row[0]' title='".get_vocab('admin_access_area.php')."'><img src=\"img_grr/restricted_s.png\" alt=\"".get_vocab('admin_access_area.php')."\" title=\"".get_vocab('admin_access_area.php')."\" align=\"middle\" border=\"0\" /></a></td>\n";
            else
                echo "<td>&nbsp;</td>\n";
            ...
            echo "</td>\n";
            echo "<td><a href=\"admin_edit_room.php?area=$row[0]\"><img src=\"img_grr/edit_s.png\" alt=\"". get_vocab("edit") ."\" title=\"".get_vocab("edit")."\" align=\"middle\" border=\"0\" /></a></td>\n";
            if(authGetUserLevel(getUserName(),$row[0],'area') >= 5)
                echo "<td><a href=\"admin_room_del.php?type=area&amp;area=$row[0]\"><img src=\"img_grr/delete_s.png\" alt=\"".get_vocab('delete')."\" title=\"".get_vocab('delete')."\" align=\"middle\" border=\"0\" /></a></td>\n";

            echo "<td><a href=\"admin_type_area.php?area_id=$row[0]\"><img src=\"img_grr/type.png\" alt=\"".get_vocab('edittype')."\" title=\"".get_vocab('edittype')."\" align=\"middle\" border=\"0\" /></td>\n";
    ...
    if(isset($area)) {
        $res = grr_sql_query("select id, room_name, description, capacity, max_booking, statut_room, max_booking_week from grr_room where area_id=$area order by order_display, room_name");
        if (! $res) fatal_error(0, grr_sql_error());
        if (grr_sql_count($res) != 0) {
            ...
            for ($i = 0; ($row = grr_sql_row($res, $i)); $i++) {
                ...
                echo "<tr><td".$color.">" . htmlspecialchars($row[1]) . " (" . htmlspecialchars($row[2]) . ", $row[4], $row[6])</td>\n";
                echo "<td><a href=\"admin_edit_room.php?room=$row[0]\"><img src=\"img_grr/edit_s.png\" alt=\"".get_vocab('edit')."\" title=\"".get_vocab('edit')."\" align=\"middle\" border=\"0\" /></a></td>\n";
                echo "<td><a href=\"admin_room_del.php?type=room&amp;room=$row[0]&amp;area_id=$area\"><img src=\"img_grr/delete_s.png\" alt=\"".get_vocab('delete')."\" title=\"".get_vocab('delete')."\" align=\"middle\" border=\"0\" /></a></td>";
