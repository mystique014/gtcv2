 ...
 * Revision 1.10  2009-06-04 15:30:17  grr
 * *** empty log message ***
 *
 * Revision 1.9  2009-04-14 12:59:17  grr
 * *** empty log message ***
 *
 * Revision 1.8  2009-04-09 14:52:31  grr
 * *** empty log message ***
 *
 * Revision 1.7  2009-02-27 13:28:19  grr
 * *** empty log message ***
 *
 ...
 * *** empty log message ***
 *
 * Revision 1.2  2008-11-05 21:47:53  grr
 * *** empty log message ***
 *
 * Revision 1.1  2008-11-05 15:05:55  grr
 * *** empty log message ***
 *
 */
 ...
 */
 ...
 function create_site($id)
 {
   ...
   if (!isset($id_site)) $id_site = isset($_POST['id']) ? $_POST['id'] :  NULL;
   ...
   if ((! (isset($_POST['save']) or isset($_GET['save']))) AND ($id_site==0))
   {
...
echo '          </td>
 ...
 function update_site($id)
 {
   ...
   {
     // Initialisation
     $res = grr_sql_query("SELECT * FROM ".$_COOKIE["table_prefix"]."_site WHERE id='".$id."'");
   ...
   // Sinon, il faut valider le formulaire
     ...
     if ((isset($_POST['save']) and ($_POST['save']!='no')) or ((isset($_GET['save'])) and ($_GET['save']!='no')))
     {
         $sql="update ".$_COOKIE["table_prefix"]."_site
       		SET sitecode='".strtoupper(protect_data_sql($sitecode))."',
       			sitename='".protect_data_sql($sitename)."',
       			adresse_ligne1='".protect_data_sql($adresse_ligne1)."',
       			adresse_ligne2='".protect_data_sql($adresse_ligne2)."',
 ...
 function delete_site($id)
 {
  if (!(isset($_GET['confirm']))) {
    ...
    if ($_GET['confirm']=='yes') {
     grr_sql_command("delete from ".$_COOKIE["table_prefix"]."_site where id='".$_GET['id']."'");
     grr_sql_command("delete from ".$_COOKIE["table_prefix"]."_j_site_area where id_site='".$_GET['id']."'");
     grr_sql_command("delete from ".$_COOKIE["table_prefix"]."_j_useradmin_site where id_site='".$_GET['id']."'");
     grr_sql_command("update ".$_COOKIE["table_prefix"]."_utilisateurs set default_site = '-1' where default_site='".$_GET['id']."'");
     $test = grr_sql_query1("select VALUE from ".$_COOKIE["table_prefix"]."_setting where NAME='default_site'");
     if ($test==$_GET['id'])
          grr_sql_command("delete from ".$_COOKIE["table_prefix"]."_setting where NAME='default_site'");
