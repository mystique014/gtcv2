<?php
#########################################################################
#                        admin_ompta_abonnes.php                         							 #
#                                                                      									 #
#            Interface générale de modification/ajout/affichage d'une fiche comptable utilisateur     	 #
#                                                                       								#
...
echo "<h2>".get_vocab('admin_compta_abonnes.php')."</h2>";
echo get_vocab('admin_compta_abonnes_explications');
?>
...
$retry='';
$msg = '';
$aff_login = isset($_GET["aff_login"]) ? $_GET["aff_login"] : NULL;
$reg_login = isset($_GET["reg_login"]) ? $_GET["reg_login"] : NULL;
...
if (empty($order_by)) { $order_by = 'date DESC'; }
?>
<p class=bold><hr>
| <a href="admin_accueil.php"><?php echo get_vocab("back"); ?></a> |
<a href="admin_compta_abonnes.php?year=<?php echo getSettingValue("default_year"); ?>"><?php echo get_vocab("fiche"); ?></a> |
<br>
</p>
<form action="admin_compta_abonnes.php?display=<?php echo $display; ?>" method='GET'>

<span class="norme">
<?php
// -----------------------------------------//Affichage du formulaire
echo "<p align='center'><b><u>".get_vocab("fiche_abonne")."</u></b></p>";
echo "<table border=\"0\" cellpadding=\"5\"><tr>";
//-----------------------//liste des abonnes
echo "<td>".get_vocab("abonnes")." ".get_vocab("deux_points")."</td>";
...
//------------------//fin liste catégorie
echo "</td></tr>\n";
echo "<td>".get_vocab("description")."</td><td><input type=\"text\" name=\"reg_description\" size=\"20\" value=\"";
...
//--------------------------------------------------------------------------------
echo "<hr><p align='center'><b><u>".get_vocab("affiche_abonne")."</u></b></p>";
echo "<br><table><td><b>Liste des fiches pour l'ann&eacute;e :</b></td><td>";
...
if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
{
    $selected = ($row[0] == $aff_login) ? "selected" : "";
    $link = "admin_compta_abonnes.php?aff_login=$row[0]&amp;year='".getSettingValue("default_year")."'";
    $out_html .= "<option $selected value=\"$link\">" . htmlspecialchars($row[1])."  ". htmlspecialchars($row[2]);
}
$out_html .= "</select>
...
function login_go()
{
box = document.forms[\"aff_login\"].aff_login;
...
// On appelle les informations de l'utilisateur pour les afficher :
    ...
    if ($res) {
		...
		echo "<td bgcolor='$bgcolor'><a href='admin_compta_abonnes.php?user_modify={$col[$i][0]}&amp;action_copy=yes&amp;user_login=$user_login&amp;year=$user_year '>".get_vocab("copy")."</a></td>";
        ...
        echo "<td><a href='admin_compta_abonnes.php?user_modify={$col[$i][0]}&amp;action_modify=yes&amp;aff_login=$aff_login&amp;year=$user_year ' onclick='return confirmlink(this, \"$reg_login\", \"$themessage\")'>".get_vocab("edit")."</a></td>";
		}
        ...
        echo "<td><a href='admin_compta_abonnes.php?user_del={$col[$i][0]}&amp;action_del=yes&amp;aff_login=$aff_login&amp;year=$user_year' onclick='return confirmlink(this, \"$reg_login\", \"$themessage\")'>".get_vocab("delete")."</a></td>";
		echo "</tr>";
		}
        }
...
//liste des fiches de tous les abonnés
...
if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
{
    ...
    $link = "admin_compta_abonnes.php?year=$row[0]";
...
// Affichage du tableau
echo "<table border=1 cellpadding=3>";
echo "<tr><td><b><a href='admin_compta_abonnes.php?order_by=date'>".get_vocab("datee")."</a></b></td>";
echo "<td><b><a href='admin_compta_abonnes.php?order_by=login'>".get_vocab("login")."</a></b></td>";
echo "<td><b><a href='admin_compta_abonnes.php?order_by=nom'>".get_vocab("name")."</a></b></td>";
echo "<td><b><a href='admin_compta_abonnes.php?order_by=mode'>".get_vocab("mode")."</a></b></td>";
echo "<td><b><a href='admin_compta_abonnes.php?order_by=description'>".get_vocab("description")."</a></b></td>";
echo "<td><b><a href='admin_compta_abonnes.php?order_by=categorie'>".get_vocab("nom_categorie")."</a></b></td>";
echo "<td><b><a href='admin_compta_abonnes.php?order_by=montant'>".get_vocab("montant")."</a></b></td>";
echo "<td><b>".get_vocab("copy")."</b></td>";
echo "<td><b>".get_vocab("edit")."</b></td>";
echo "<td><b>".get_vocab("delete")."</b></td>";
echo "</tr>";
...
// On appelle les informations de l'utilisateur pour les afficher :
    ...
    if ($res) {
		...
		$col[$i][6] = $nom_categorie;
		$col[$i][7] = $user_montant;
		$total += $col[$i][7];
		$col[$i][8] = $user_year ;
		...
		//affichage des cellules
		echo "<tr><td bgcolor='$bgcolor'>{$col[$i][3]}</td>";
		echo "<td bgcolor='$bgcolor'>{$col[$i][1]}</td>";
		echo "<td bgcolor='$bgcolor'>{$col[$i][2]}</td>";
		echo "<td bgcolor='$bgcolor'>{$col[$i][4]}</td>";
		echo "<td bgcolor='$bgcolor'>{$col[$i][5]}</td>";
		echo "<td bgcolor='$bgcolor'>{$col[$i][6]}</td>";
		echo "<td bgcolor='$bgcolor' align='right'>{$col[$i][7]}</td>";
        ...
        $themessage = get_vocab("confirm_edit");
        echo "<td><a href='admin_compta_abonnes.php?user_modify={$col[$i][0]}&amp;action_modify=yes&amp;year=$user_year ' onclick='return confirmlink(this, \"$themessage\")'>".get_vocab("edit")."</a></td>";
		}
        ...
        $themessage = get_vocab("confirm_del");
        echo "<td><a href='admin_compta_abonnes.php?user_del={$col[$i][0]}&amp;action_del=yes&amp;year=$user_year' onclick='return confirmlink(this, \"$themessage\")'>".get_vocab("delete")."</a></td>";
		echo "</tr>";
		}
        }
		echo "<tr><td></td><td></td><td></td><td></td><td align='right'><b>".get_vocab("solde")."</b></td>";
echo "<td align='right'><b>$total</b></td></tr>";
    }
echo "</table>";
}
