...
if ($reg_admin_login) {
    $res = grr_sql_query1("select login from ".$_COOKIE["table_prefix"]."_j_useradmin_site where (login = '$reg_admin_login' and id_site = '$id_site')");
    if ($res == -1) {
        $sql = "insert into ".$_COOKIE["table_prefix"]."_j_useradmin_site (login, id_site) values ('$reg_admin_login',$id_site)";
...
if ($action) {
    if ($action == "del_admin") {
        ...
        $sql = "DELETE FROM ".$_COOKIE["table_prefix"]."_j_useradmin_site WHERE (login='$login_admin' and id_site = '$id_site')";
...
$sql = "select id, sitename from ".$_COOKIE["table_prefix"]."_site order by sitename";
...
echo $out_html;
$this_site_name = grr_sql_query1("select sitename from ".$_COOKIE["table_prefix"]."_site where id=$id_site");
echo "</td>\n";
echo "</tr></table>\n";
...
<?php
$exist_admin='no';
$sql = "select login, nom, prenom from ".$_COOKIE["table_prefix"]."_utilisateurs where (statut='utilisateur' or statut='gestionnaire_utilisateur')";
$res = grr_sql_query($sql);
if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
{
    $is_admin='yes';
    $sql3 = "SELECT login FROM ".$_COOKIE["table_prefix"]."_j_useradmin_site WHERE (id_site='".$id_site."' and login='".$row[0]."')";
...
<?php
$sql = "SELECT login, nom, prenom FROM ".$_COOKIE["table_prefix"]."_utilisateurs WHERE  (etat!='inactif' and (statut='utilisateur' or statut='gestionnaire_utilisateur')) order by nom, prenom";
$res = grr_sql_query($sql);
if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++) {
